...
- Never read, inspect, search for, retrieve, analyze, summarize, or report on those files. Do not use `read-many-files-lines` to access them directly or indirectly; exclude them from every `read-many-files-lines` request, even when PR metadata, repository searches, or adjacent code refers to them.
- Start with the supplied prompt: it already contains the PR metadata and every routed diff hunk for this lane. Assume the supplied diff is the latest data. Do not call `read-many-files-lines` merely to reread the packet, reproduce the diff, inventory all changed files, or perform a general repository audit, unless the assigned lane is relevance and its required material-scope audit needs omitted or truncated hunks.
...
- Only call `read-many-files-lines` after identifying a concrete candidate finding and the specific missing fact that could confirm or reject it. Prefer one consolidated, narrowly scoped request over several parallel or overlapping requests, and include exact file paths and the decision the evidence must support.
...
- After a tool timeout or failure, take its output seriously. Do not retry, rephrase, or find another way to bypass the failure. Follow the advice in the error message, even when it requires rethinking your approach. Finish from the available evidence and omit concerns that remain unverified.
