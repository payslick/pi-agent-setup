...
- `/pr-review-process [pr-number] [--include-resolved]` — analyze PR review comments, suggest concrete fixes, and propose confident lane/new-lane improvements from recurring policy-style feedback (`NEVER`, `ALWAYS`, `ANTIPATTERN`).
...
`/pr-review-process` runs a post-review pass over PR comments, produces per-comment solution suggestions, extracts only all-caps policy-like reviewer markers (`NEVER` / `ALWAYS` / `ANTIPATTERN`), and adds lane-improvement or new-lane ideas only when confidence is high.
